...
MainWindow::MainWindow(QWidget *parent)
	...
	, m_obstacleAvoiderThread(new QThread(this))
	, m_robotStartupLocation(false)
	, m_obstacleAvoider(new ObstacleAvoider(this))
{
	...
	connect(this, &MainWindow::handleTrajectory, m_obstacleAvoider, &ObstacleAvoider::on_handleTrajectory_handle, Qt::QueuedConnection);
	connect(m_obstacleAvoider, &ObstacleAvoider::requestTrajectory, this, &MainWindow::on_requestTrajectoryCalculate, Qt::QueuedConnection);

	// Starting threads
	m_controllerThread->start();
	m_plannerThread->start();
	m_obstacleAvoiderThread->start();
	...
	m_obstacleAvoider->moveToThread(m_obstacleAvoiderThread);
}
...
int MainWindow::processThisRobot(TKobukiData robotdata)
{
	...
	emit handleTrajectory(copyOfLaserData,m_x,m_y,m_fi);
...
void MainWindow::on_requestTrajectoryCalculate(double x, double y, RobotTrajectoryController::MovementType type)
{
	m_xTarget = x;
	m_yTarget = y;
	qDebug() << "Calculating trajectory with obstacle avoidance x: " << x << " y:" << y;
	_calculateTrajectory(type);
}
...
void MainWindow::onLiveAvoidObstaclesButton_clicked(bool clicked)
{
	...
	if (ok1 && ok2) {
		emit requestAvoidance(m_xTarget, m_yTarget);
	}
}
