...
unsigned char emptyMessage[]={0x1A,0x01,0x0f,0x00,0x00,0x00,0x00,0x00,0x01,0x02,0x03,0x04,0x00,0x00,0x00,0x00,0x00,0x00,0x04,0x07,0x01,0x02,0x00,0x00,0x00,0x00,0x00,0x00};
...
void MainWindow::paintEvent(QPaintEvent *event)
{
    ...
    for(int i=0;i<mapa.obstacle.size();i++)
    {
        ...
        for(int j=0;j<mapa.obstacle[i].points.size();j++)
        {
            ...
            int ymax=rect.height()-rect.height()*(mapa.obstacle[i].points[(j+1)%mapa.obstacle[i].points.size()].point.y-mapload.minY)/(mapload.maxY-mapload.minY);
    ...
    painter.drawLine(rect.x()+xrobot,rect.y()+yrobot,rect.x()+xrobot+xpolomer*cos((360-robotik.fi)*3.14159/180),rect.y()+((yrobot+ypolomer*sin((360-robotik.fi)*3.14159/180))));
    ...
    if(updateLaserPicture==1)
    {
        ...
        for(int k=0;k<copyOfLaserData.numberOfScans;k++)
        {
            ...
            int xp=rect.x()+xrobot+rect.width()*(dist*cos((robotik.fi+360.0-copyOfLaserData.Data[k].scanAngle)*3.14159/180.0))/(mapload.maxX-mapload.minX);
            int yp=rect.y()+yrobot+(-(rect.height()*(dist*sin((robotik.fi+360.0-copyOfLaserData.Data[k].scanAngle)*3.14159/180.0))/(mapload.maxY-mapload.minY)));
...
void MainWindow::laserSimulator()
{
    ...
    while(stopAll==0)
    {
        ...
        // if(((std::chrono::duration_cast<std::chrono::microseconds>(second-first)).count())>505)
        {
            ...
            angle = angle +1.3;//(cas/frequencyBetweenpoints); //cas medzi vzorkami a frequencyBetweenpoints je preratane na kolko otacok za sekundu mame... ak by bolo 5.5,tak nam vyjde jeden stupen
            first=second;
            ...
            if(angle>360.0)
            {
               ...
               // memcpy( &copyOfLaserData,&localLaser,sizeof(LaserMeasurement));
                ...
                if(hasLaserAddress==1)
                    returnval=  laserRecv->writeDatagram((const char*)&copyOfLaserData.Data,sizeof(LaserData)*copyOfLaserData.numberOfScans,laseraddress,/*52999*/port);
...
double MainWindow::findLaserPoint(TMapArea &mapicka,sim_robot robotik,double angle)
{
    ...
    {
        ret=lineIntersection(mapicka.wall.points[i].point.x,mapicka.wall.points[i].point.y,mapicka.wall.points[(i+1)%mapicka.wall.points.size()].point.x,mapicka.wall.points[(i+1)%mapicka.wall.points.size()].point.y,robotik.x,robotik.y,vzdpx,vzdpy);
        if(ret>0)
        {
            if(dist>ret)
            {
    ...
    for(int i=0;i<mapicka.obstacle.size();i++)
    {
        ...
        {
            ret=lineIntersection(mapicka.obstacle[i].points[j].point.x,mapicka.obstacle[i].points[j].point.y,mapicka.obstacle[i].points[(j+1)%mapicka.obstacle[i].points.size()].point.x,mapicka.obstacle[i].points[(j+1)%mapicka.obstacle[i].points.size()].point.y,robotik.x,robotik.y,vzdpx,vzdpy);
            if(ret>0)
            {
                if(dist>ret)
                {
...
bool MainWindow::isObstacleInFronOfObstacle(int first,int second)
{
    ...
    for (int i=0;i<mapa.obstacle[firstID].points.size();i++)
    {
        ...
        for(int j=0;j<mapa.obstacle[secondID].points.size();j++)
        {
            ...
            if(isInWay(robotik.x,robotik.y,mapa.obstacle[firstID].points[i].point.x,mapa.obstacle[firstID].points[i].point.y,mapa.obstacle[firstID].points[nextI].point.x,mapa.obstacle[firstID].points[nextI].point.y,mapa.obstacle[secondID].points[j].point.x,mapa.obstacle[secondID].points[j].point.y,mapa.obstacle[secondID].points[nextJ].point.x,mapa.obstacle[secondID].points[nextJ].point.y)==true)
            {
...
void MainWindow::generateAndSentImage()
{
    ...
    for (auto it =  orgn.rbegin(); it != orgn.rend(); ++it)
    {
        ...
        if(*it<=0)
        {
            ...
            dst= sqrt(pow((mapa.wall.points[(sur+1)%mapa.wall.points.size()].point.x-campos.x),2)+pow((mapa.wall.points[(sur+1)%mapa.wall.points.size()].point.y-campos.y),2));
            angl=(atan2((mapa.wall.points[(sur+1)%mapa.wall.points.size()].point.y-campos.y),(mapa.wall.points[(sur+1)%mapa.wall.points.size()].point.x-campos.x))*180.0/3.14159  )- robotik.fi;
        ...
        else
        {
            ...
            for(int i=0;i<mapa.obstacle[obstc].points.size();i++)
            {
                ...
                double angl=(atan2((mapa.obstacle[obstc].points[sur].point.y-campos.y),(mapa.obstacle[obstc].points[sur].point.x-campos.x))*180.0/3.14159  )- robotik.fi;
                ...
                angl=(atan2((mapa.obstacle[obstc].points[sur].point.y-campos.y),(mapa.obstacle[obstc].points[sur].point.x-campos.x))*180.0/3.14159  )- robotik.fi;
...
double MainWindow::lineIntersection(double X1, double Y1,double X2,double Y2,double Rx,double Ry,double Rx2,double Ry2)
{
...

    {
        ...
        t=(u1*Ay+u2*Cx-Ax*u2-Cy*u1)/(v2*u1-u2*v1);  //vyratame bod prieniku (ak je prienik useciek ,tak musi byt s intervalu 0 -1, ak je mimo tak je to prienik priamok)
...
bool MainWindow::isRobotInCollision(TMapArea &mapicka,sim_robot robotik,double safedist)
{
    ...
    for(int i=0;i<mapicka.wall.points.size();i++)
    {
        ret=robotObstacleDistance(mapicka.wall.points[i].point.x,mapicka.wall.points[i].point.y,mapicka.wall.points[(i+1)%mapicka.wall.points.size()].point.x,mapicka.wall.points[(i+1)%mapicka.wall.points.size()].point.y,robotik.x,robotik.y);
        if(ret<safedist)
            return true;
    }
    ...
    for(int i=0;i<mapicka.obstacle.size();i++)
    {
        ...
        for(int j=0;j<mapicka.obstacle[i].points.size();j++)
        {
            ret=robotObstacleDistance(mapicka.obstacle[i].points[j].point.x,mapicka.obstacle[i].points[j].point.y,mapicka.obstacle[i].points[(j+1)%mapicka.obstacle[i].points.size()].point.x,mapicka.obstacle[i].points[(j+1)%mapicka.obstacle[i].points.size()].point.y,robotik.x,robotik.y);
            if(ret<safedist)
                return true;
        }
    }
...
void MainWindow::robotSimExec(std::vector<unsigned char> mess)
{
    ...
    if(isRobotInCollision(mapa,temp_robot,20)==false)
    {
        ...
        robotik.encoderRightA=robotik.encoderRightA>65535.0 ? robotik.encoderRightA-65536.0: robotik.encoderRightA<0 ? robotik.encoderRightA+65536 :robotik.encoderRightA;
        robotik.encoderLeftA=robotik.encoderLeftA>65535.0 ? robotik.encoderLeftA-65536.0: robotik.encoderLeftA<0 ? robotik.encoderLeftA+65536.0 :robotik.encoderLeftA;
